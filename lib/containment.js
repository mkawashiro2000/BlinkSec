...
function resolveRuleId(accion, ruleId) {
  ...
  // El id acaba interpolado en la URL de reversión de WF-07. Se exige la forma
  // que realmente usa Cloudflare (hex/alfanumérico) para que un valor
  // inesperado no pueda inyectar '/' o '?' y desviar la petición a otra ruta
  // de la API. Ante algo raro se deja el placeholder: la reversión fallará de
  // forma visible en vez de golpear un endpoint equivocado en silencio.
  if (!/^[A-Za-z0-9_-]{1,128}$/.test(String(ruleId))) return accion;
  return {
    ...accion,
    undo_payload: {
      ...accion.undo_payload,
