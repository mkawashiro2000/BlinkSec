 ...
 *
 * Dos niveles de acceso, dos formas de respuesta MUY distintas del mismo
 * proveedor:
 *
 *   - De pago (endpoint /v3/ip/{ip}): objetos anidados
 *     internet_scanner_intelligence / business_service_intelligence, con
 *     metadatos ricos (JA4, CVEs, puertos de destino...).
 *   - Community (endpoint /v3/community/{ip}, nivel gratuito): objeto plano
 *     { classification, riot, noise, name, link, last_seen }.
 *
 * `parseGreyNoise` detecta cuál llegó por la FORMA de la respuesta, no por
 * configuración: así, si la cuenta pasa de Community a un tier de pago más
 * adelante, sólo hay que cambiar la URL en el nodo HTTP — el parser se
 * adapta solo, sin tocar código. Ambas rutas normalizan al mismo contrato de
 * salida (`verdict`, `data.kind`), así que el motor de scoring nunca necesita
 * saber cuál de las dos respondió.
 */
function parseGreyNoise(res, httpStatus = 200) {
  ...
  // probablemente dirigido. No es un error. Aplica a ambos niveles.
  if (httpStatus === 404) {
  ...
  // La forma de pago anida el resultado en estos dos objetos; su ausencia
  // (aun con res.riot / res.noise como booleanos sueltos) indica Community.
  const esFormaDePago = 'internet_scanner_intelligence' in res || 'business_service_intelligence' in res;

  return esFormaDePago ? parseGreyNoisePago(res) : parseGreyNoiseCommunity(res);
}

function parseGreyNoisePago(res) {
  const business = res.business_service_intelligence ?? {};
  const scanner = res.internet_scanner_intelligence ?? {};
 ...
 *
 * Forma de la respuesta: { ip, noise, riot, classification, name, link,
 * last_seen, message }. `riot` cubre lo mismo que business_service_intelligence
 * en el tier de pago (infraestructura legítima conocida); `noise` cubre lo
 * mismo que internet_scanner_intelligence (ruido de escaneo de internet).
 * No trae los metadatos enriquecidos (JA4, CVEs, puertos) del tier de pago —
 * `data.limited` lo deja constancia explícita para quien lea el ticket.
 */
function parseGreyNoiseCommunity(res) {
  if (res.riot === true) {
    return wrap('greynoise', {
      available: true,
      verdict: 'benign',
      data: { kind: 'business_service', name: res.name ?? null, limited: true },
    });
  }

  if (res.noise === true) {
    const clasificacion = res.classification; // benign | malicious | unknown
    const verdictMap = { benign: 'benign', malicious: 'malicious' };
    return wrap('greynoise', {
      available: true,
      verdict: verdictMap[clasificacion] ?? 'inconclusive',
      data: { kind: 'internet_scanner', classification: clasificacion ?? null, limited: true },
    });
  }

  return wrap('greynoise', { available: true, verdict: 'not_observed', data: { seen: false, limited: true } });
}
