 ...
 *
 * Convierte los esquemas heterogéneos de Wazuh, Splunk y Elastic en un único
 ...
 * cuarto SIEM significa escribir un normalizador aquí y nada más.
 *
...
function normalizeWazuh(a) {
  const ips = [];
  const srcIp = firstOf(a, ['data.srcip', 'data.src_ip', 'data.win.eventdata.ipAddress']);
  const dstIp = firstOf(a, ['data.dstip', 'data.dst_ip']);
  if (isPublicIPv4(srcIp)) ips.push({ value: srcIp, direction: 'src' });
  if (isPublicIPv4(dstIp)) ips.push({ value: dstIp, direction: 'dst' });

  const hashes = [];
  for (const p of ['syscheck.md5_after', 'syscheck.sha1_after', 'syscheck.sha256_after', 'data.hash', 'data.md5', 'data.sha256']) {
    const h = classifyHash(dig(a, p));
    if (h) hashes.push(h);
  }
  ...
  return {
    source: 'wazuh',
    rule: {
      id: String(firstOf(a, ['rule.id']) ?? ''),
      name: firstOf(a, ['rule.description']) ?? '',
      level: Number(firstOf(a, ['rule.level']) ?? 0),
      mitre: dig(a, 'rule.mitre.id') ?? [],
    },
    asset: {
      host: firstOf(a, ['agent.name', 'manager.name']) ?? '',
      ip: firstOf(a, ['agent.ip']) ?? null,
    },
    identity: {
      user: firstOf(a, ['data.dstuser', 'data.srcuser', 'data.win.eventdata.targetUserName']) ?? null,
      domain: firstOf(a, ['data.win.eventdata.targetDomainName']) ?? null,
    },
    artifacts: { ips, hashes, domains: [], urls: [] },
    eventTimestamp: firstOf(a, ['timestamp']) ?? null,
  };
}
...
const NORMALIZERS = { wazuh: normalizeWazuh, splunk: normalizeSplunk, elastic: normalizeElastic };
...
/**
 * @param {string} source  'wazuh' | 'splunk' | 'elastic'
